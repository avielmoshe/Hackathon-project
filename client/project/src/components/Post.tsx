    ...
    <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-gray-900 dark:to-gray-800 shadow-lg rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-700 transform transition-transform hover:scale-105 duration-300">
      ...
      <div className="flex items-center space-x-4 mb-4">
        <div
          ...
          style={{ backgroundImage: `url(${post.profile})` }}
        ></div>
        <div>
          <h2 className="text-2xl font-extrabold text-gray-800 dark:text-gray-100">{post.title}</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">{post.providerType}</p>
        </div>
      </div>
      ...
      {post.location && (
        ...
        >
          ...
          <span><strong>Location:</strong> {post.location.join(", ")}</span>
        </div>
      )}
      ...
      {post.serviceType && (
        <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-2">
          ...
          <span><strong>Services:</strong> {post.serviceType.join(", ")}</span>
        </div>
      )}
      ...
      {post.status && (
        <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-2">
          ...
          <span><strong>Status:</strong> {post.status.join(", ")}</span>
        </div>
      )}
